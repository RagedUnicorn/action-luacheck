...
This document describes how to cut a new release of `action-luacheck`.
...
[`docker-luacheck`](https://github.com/RagedUnicorn/docker-luacheck) image tag. GitHub Actions does
not allow `${{ inputs.* }}` interpolation in `runs.image`, so the luacheck version is locked per
action release.

Bumping luacheck (or the underlying Alpine base image) requires:

1. A new release in `docker-luacheck` published to GHCR.
...
6. The release workflow runs against the pushed `vX.Y.Z` tag and produces a GitHub Release with
   ...
   GitHub UI to add a **Breaking changes** section and the pinned `docker-luacheck` image tag.
